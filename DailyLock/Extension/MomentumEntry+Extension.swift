...
extension MomentumEntry {
    static var samples: [MomentumEntry] {
        [
            MomentumEntry(date: Date().addingTimeInterval(-86400 * 30*2), detail: "Lost motivation and skipped tasks.", sentiment: .indifferent, lockedAt: Date().addingTimeInterval(-86400 * 30*2)),
            ...
            MomentumEntry(date: Date().addingTimeInterval(-86400 * 1), detail: "Lost motivation and skipped tasks.", sentiment: .negative, lockedAt: Date().addingTimeInterval(-86400 * 1)),
            MomentumEntry(date: Date(), detail: "Lost motivation and skipped tasks.", sentiment: .negative, lockedAt: Date.now)
        ]
    }
}
